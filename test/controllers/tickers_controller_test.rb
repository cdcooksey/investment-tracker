...
class TickersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create ticker" do
    assert_difference('Ticker.count') do
      post tickers_url, params: { ticker: { enabled: @ticker.enabled, name: @ticker.name, user_id: @ticker.user_id } }
    end
  ...
  test "should update ticker" do
    patch ticker_url(@ticker), params: { ticker: { enabled: @ticker.enabled, name: @ticker.name, user_id: @ticker.user_id } }
